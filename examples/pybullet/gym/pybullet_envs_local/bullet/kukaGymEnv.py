...
class KukaGymEnv(gym.Env):
  ...
  def reset(self):
    ...
    self.terminated = 0
    p.resetSimulation() # wipe the whole world
    ...
    # create block in a random position with random orientation
  ...
  def step(self, action):
    """ action is an array of motor commands  """
    if (self._isDiscrete):
      dv = 0.005
  ...
  def step2(self, action):
    ...
    for i in range(self._actionRepeat):
      self._kuka.applyAction(action) # set the joint angles
      p.stepSimulation()
