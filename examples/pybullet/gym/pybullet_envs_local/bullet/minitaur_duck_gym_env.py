...
class MinitaurBulletDuckEnv(gym.Env):
    ...
    if not self._torque_control_enabled:
      for _ in range(100):
        if self._pd_control_enabled or self._accurate_motor_model_enabled:
          self.minitaur.ApplyAction([math.pi / 2] * 8) # the mintaur has 8 motors
  ...
  def _transform_action_to_motor_command(self, action):
    ...
    # if using leg model, constrain angles to be -1 <= angle <= 1 radian
    if self._leg_model_enabled:
      ...
      # assume actions are leg angles: convert from leg angles to motor angles
  ...
  def _termination(self):
    ...
    distance = math.sqrt(position[0]**2 + position[1]**2) # sqrt(x^2 + y^2) = distance from origin on the x-y plane
  ...
  def _reward(self):
    ...
    shake_reward = -abs(current_base_position[2] - self._last_base_position[2]) # shake in the z axis
    self._last_base_position = current_base_position
    
    # torque X velocity = what?
    energy_reward = np.abs(
        np.dot(self.minitaur.GetMotorTorques(),
               self.minitaur.GetMotorVelocities())) * self._time_step
    
    # combine rewards together 
