...
class MotorModel(object):
  ...
  def convert_to_torque(self, motor_commands, current_motor_angle, current_motor_velocity):
    ...
    # determie pwm from current and desired angles and velocity
    else:
      pwm = (-self._kp * (current_motor_angle - motor_commands) -
             self._kd * current_motor_velocity)
    pwm = np.clip(pwm, -1.0, 1.0) # -1 full spped backward rotation, 1 full speed forward rotation
  ...
  def _convert_to_torque_from_pwm(self, pwm, current_motor_velocity):
    ...
    current = voltage_net / self._resistance # ohm's law
    current_sign = np.sign(current)
    current_magnitude = np.absolute(current)

    # Saturate torque based on empirical current relation.
    actual_torque = np.interp(current_magnitude, self._current_table, self._torque_table) # map current to torque
